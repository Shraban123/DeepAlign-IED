"""
Methodology: Dividing tweet into blocks
This file splits the Twitter dataset into 21 message blocks, 
use the message blocks to construct heterogeneous social graphs 
along with the homogeneous message graph.

Note that:
# 1) We adopt the Latest Message Strategy (which is the most efficient and gives the strongest performance. Here, 
...
def construct_graph_from_df(df, G=None):
    ...
    for _, row in df.iterrows():
        ...
        G.add_node(tid)
        G._node[tid]['tweet_id'] = True  # right-hand side value is irrelevant for the lookup
...
# To use all the messages, set test=False, and the number of messages in the message blocks will our block design of the paper.
...
message, data_split, all_graph_mins = construct_incremental_dataset_0922(df, save_path, f, True) # changed by XXXX-1 - This statement does testing
# message, data_split, all_graph_mins = construct_incremental_dataset_0922(df, save_path, f, False) # changed by XXXX-1 - This statement does training

...
class Transformer(nn.Module):
    ...
    def forward(self, x, kv=None, self_padding_mask=None, self_attn_mask=None,
        ...
        for idx, layer in enumerate(self.layers):
            x, _,  = layer(x, kv, self_padding_mask, self_attn_mask, external_memories, external_padding_mask)
        return x
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, key_padding_mask=None, attn_mask=None, need_weights=False):
        ...
        """
        #print(f"DEBUG: query size: {query.size()}, key size: {key.size()}, value size: {value.size()}")
